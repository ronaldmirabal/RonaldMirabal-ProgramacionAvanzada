...
public class Exercise1Week4 {
    ...
    public IceScream createIceScreamCombination(List<IceScream> iceScreams, Double combinationCost) {
        return IceScream.builder()
                ...
                .flavor("MIXTO")
                .amount(calculateIceScreamPrice(iceScreams, combinationCost))
                .build();

    }


    private BigDecimal calculateIceScreamPrice(List<IceScream> iceScreams, Double combinationCost){
        ...
        for (IceScream iceScream : iceScreams){
            if (result.doubleValue() == -1){
                ...
                BigDecimal percentageInDecimal = new BigDecimal(combinationCost).divide(new BigDecimal(100),2,RoundingMode.CEILING);
                result = result.add(iceScream.getAmount().multiply(percentageInDecimal));
            }
        }
